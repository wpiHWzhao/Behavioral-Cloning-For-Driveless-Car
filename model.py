...
# Read data path from the csv file
def readData():
    samples = []
...
# Generator is used so data would not eat up the memory when training the network. 
    ...
    while 1: # Loop forever so the generator never terminates
        ...
        for offset in range(0, num_samples, batch_size):
            ...
            for batch_sample in batch_samples:
                ...
                # Simulation use RGB image as input.
                center_image = cv2.cvtColor(center_image, cv2.COLOR_BGR2RGB) 
                images.append(center_image)
                angles.append(center_angle)
                # Images are filped to produce more data
                ...
                # Apply a correction term so the car can recover from diverging. 
...
def nVaidiaModel():
    ...
    # Only keep the parts that capture the road. 
